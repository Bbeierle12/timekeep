...
export const attestationService = {
  ...
  async sign(input: AttestationInput) {
    ...
    try {
      ...
      await dailySummaryService.recalculate(input.employeeId, input.workDate, client);

      if (triggersPremium) {
        await dailySummaryService.flagPremiumPay(input.employeeId, input.workDate, true, 1, client);
      }
